...
def find_root_dir() -> str:
    ...
    for start in candidates:
        ...
        curr = start
        for _ in range(6):
            if os.path.exists(os.path.join(curr, "apps", "streamer", "server")):
...
class ControllerManager:
    def __init__(self):
        ...
        self.server_dir  = os.path.join(self.root_dir, "apps", "streamer", "server")
        self.backend_dir = os.path.join(self.root_dir, "apps", "agent", "server")
        self.mobile_dir  = os.path.join(self.root_dir, "apps", "mobile", "app")
...
def run_stream_subcommand():
    ...
    server_dir     = os.path.join(root_dir, "apps", "streamer", "server")
    agent_core_dir = os.path.join(root_dir, "packages", "core")
    protocol_dir   = os.path.join(root_dir, "packages")
    infra_dir      = os.path.join(root_dir, "infrastructure")

    for _d in (server_dir, agent_core_dir, protocol_dir, infra_dir):
...
def run_backend_subcommand():
    root_dir = find_root_dir()
    backend_dir    = os.path.join(root_dir, "apps", "agent", "server")
    agent_core_dir = os.path.join(root_dir, "packages", "core")
    protocol_dir   = os.path.join(root_dir, "packages")
    infra_dir      = os.path.join(root_dir, "infrastructure")

    for _d in (backend_dir, agent_core_dir, protocol_dir, infra_dir):
        if _d not in sys.path:
            sys.path.insert(0, _d)
