"""apps/agent/server — composition root for the PC Remote Agent.

This is the ONLY file in the agent server that imports adapter classes.
...
already speaks. Domain logic lives in packages/core (agent_core).

Monorepo layout (relative to repo root):
  packages/core/          — agent_core domain package
  packages/protocol/http/ — FastAPI HTTP routers
  packages/protocol/websocket/ — WebSocket router
  infrastructure/networking/   — mDNS / IP discovery
...
# Add packages/core, packages/protocol, and infrastructure to sys.path so
# they are importable without `pip install -e` in dev and frozen builds.
# ---------------------------------------------------------------------------
...
for _pkg in (
    ...
    os.path.join(_REPO_ROOT, "packages"),               # protocol.http, protocol.websocket
    os.path.join(_REPO_ROOT, "infrastructure"),         # infrastructure.networking, .logging
    _REPO_ROOT,                                         # fallback root
):
