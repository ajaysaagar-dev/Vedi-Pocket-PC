...
export interface PairedDevice {
  ...
  hostname: string;
  /**
   * NEW: stable "easy-connect" token issued alongside the per-device
   * token by the agent's `/pair` endpoint. Survives agent restarts
   * (persisted on the PC) and lets the mobile app reconnect without
   * re-entering the PIN. Optional so older paired devices that were
   * cached before this field existed still load cleanly.
   */
  commonToken?: string;
}
