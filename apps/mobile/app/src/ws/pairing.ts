...
export async function pairWithHost(
  ...
  const safeIp = cleanIp(ip);
  // NEW: prefer the server-supplied `common_token` so the same mobile
  // app can reconnect to this PC without a PIN round-trip. Older
  // servers (and the screen-stream fallback path below) won't include
  // this field; in that case we just leave `commonToken` unset.
  const commonToken: string | undefined =
    typeof pairJson?.common_token === 'string' && pairJson.common_token.length >= 16
      ? pairJson.common_token
      : undefined;
