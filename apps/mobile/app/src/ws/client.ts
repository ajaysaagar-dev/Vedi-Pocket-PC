...
class PCRemoteWSClient {
  ...
  connect(device: PairedDevice) {
    ...
    // NEW: prefer the agent's stable "common" token when one was
    // cached. The control backend persists this token across restarts
    // (see MemoryTokenStore in agent-core), so the same token is
    // reusable as long as the agent's common_token.txt file still
    // exists. We always keep the per-device `token` as the fallback
    // for older pairings and for the screen-stream server, which
    // historically had no common-token concept.
    const preferredToken =
      typeof safeDevice.commonToken === 'string' && safeDevice.commonToken.length >= 16
        ? safeDevice.commonToken
        : safeDevice.token;
    ...
    if (
      ...
      this.currentDevice.commonToken === safeDevice.commonToken
    ) {
      return;
    }
    ...
    const url = `ws://${safeDevice.ip}:${safeDevice.port}/ws?token=${encodeURIComponent(preferredToken)}`;
    ...
    socket.onopen = () => {
      ...
      // We use the per-device token here on purpose: even when we
      // connected with the common token (which is shared across all
      // paired devices on this PC), the in-memory pair-token is still
      // valid for `/status` and the WS handshake on the agent side.
      this.send({ type: 'auth', token: preferredToken });
      this.startHeartbeat();
    };
