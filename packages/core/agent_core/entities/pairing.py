...
class PairResult:
    ...
    `common_token` is the optional stable "easy-connect" token issued
    alongside the per-device token (or `None` if the implementation
    doesn't support a common token). The mobile app caches it so the
    user can re-connect to the same PC without re-entering the PIN.
    """

    accepted: bool
    device_token: Optional[SessionToken] = None
    reason: Optional[str] = None
    common_token: Optional[SessionToken] = None
