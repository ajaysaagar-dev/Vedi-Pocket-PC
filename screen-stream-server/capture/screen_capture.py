...
def _ensure_windows_desktop_access() -> None:
    """Attaches the current thread to the active user desktop on Windows."""
...
class ScreenCapturer:
    """High-performance screen capturer using MSS and Pillow."""
    ...
    def capture_frame(
        ...
        """Captures a single screen frame, overlays the mouse cursor, and encodes to JPEG bytes.
        ...
        with mss_factory() as sct:
            ...
            # Fallback if monitor_index out of bounds
            ...
            # Draw hardware mouse cursor on captured desktop image if running on Windows
            if sys.platform == "win32":
                try:
                    ...
                    if user32.GetCursorInfo(ctypes.byref(ci)) and (ci.flags & 1):
                        ...
                        if 0 <= mcx < (orig_w * scale_x) and 0 <= mcy < (orig_h * scale_y):
                            # Draw clean cursor pointer arrow
            ...
            # JPEG Encoding directly to memory buffer
            buf = io.BytesIO()
